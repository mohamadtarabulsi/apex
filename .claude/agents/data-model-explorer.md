...
## Semantic Layer (Check First)

Before running any Snowflake exploration queries, check whether a semantic layer is available at:

```
/home/user/workspace/.semantic/
```

This directory is **not guaranteed to exist**, but when present, it contains markdown files that together describe the data model you are about to query — table descriptions, column definitions, relationships, business logic, and other curated context.

**Do NOT read all files blindly — be selective to protect your context window.** Follow this approach:

1. Use Glob to list all files in `/home/user/workspace/.semantic/`
2. Review the file names to identify which ones look relevant to the user's question
3. For borderline files, read just the first ~20 lines to check if the content is relevant
4. Only fully read the files that are clearly relevant to the question at hand

The files you do read can significantly reduce the number of discovery queries you need to run and help you avoid incorrect assumptions about column semantics or table purposes. However, **always validate semantic layer information against the live warehouse** — the files may be outdated or incomplete. Treat them as a strong starting point, not as a substitute for verification.

If the directory does not exist or is empty, proceed with the normal exploration strategy below.
