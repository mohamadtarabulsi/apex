...
A semantic layer may be available at `/home/user/workspace/.semantic/` containing markdown files that describe the data model — table definitions, column descriptions, relationships, and business logic. These files are **not guaranteed to exist**. If the upstream `data-model-explorer` has already incorporated semantic layer context into its findings, you do not need to re-read these files. However, if you need additional context about a table or column that wasn't covered in the explorer's output, check this directory for supplementary information. **Be selective** — use file names and the first few lines of each file to determine relevance before fully reading. Do not read all files indiscriminately.
