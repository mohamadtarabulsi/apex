...
Note: A semantic layer may be available at `/home/user/workspace/.semantic/` in the form of markdown files that together describe the data model — table definitions, column descriptions, relationships, and business logic. These files are **not guaranteed to exist**, but when present, they provide valuable pre-curated context about the warehouse. The `data-model-explorer` subagent is already instructed to check for and use these files, so no extra action is needed on your part — just be aware they may inform the explorer's output.
